...
const buttons=document.querySelectorAll('.hero-pagination span');
buttons.forEach((btn) => {
btn.addEventListener('click',()=>{
    heroSplide.go(Number(btn.dataset.slide));
})  ;  
});
...
//hero splide
//best-seller start
const cats = $('.best-sellers .categories span');
...
$.ajax({
    ...
    $('.products-box').addClass('switch')
    $('.loader-box').removeClass('d-none')
    $('.loader-box').addClass('d-flex')

setTimeout(()=>{
    $('.loader-box').addClass('d-none')
    $('.products-box').removeClass('switch')
    $('.products-box').html(items)
},500)
    },
    error: function(err) {
...
$('.read-more-box').click(function (e) {
    ...
    let moreText = $(this).prev('p').find('.more-text');
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const latestSplide = new Splide('#latestSplide', {
        type: 'slide',
        perPage: 4,
        perMove: 1,
        gap: '24px',
